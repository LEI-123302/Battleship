...
class FleetTest {
    ...
    void testAddShip_Failure_CollisionRisk() {
        ...
        // Cria barco em (1,3), que é adjacente a (1,2)
        TestShip shipClose = new TestShip("Close", Compass.EAST, new Position(1, 3));
        shipClose.addTestPosition(new Position(1, 3));

        // colisionRisk(shipClose) deve retornar true
    ...
    void testAddShip_Failure_Overlap() {
        ...
        // Cria barco que também ocupa (1,2)
        TestShip shipOverlap = new TestShip("Overlap", Compass.EAST, new Position(1, 2));
        shipOverlap.addTestPosition(new Position(1, 2));

        // A lógica de tooCloseTo (chamada por colisionRisk) deve apanhar isto
        // porque (1,1) é adjacente a (1,2)
        assertFalse(fleet.addShip(shipOverlap), "Não deve adicionar barco sobreposto");
        assertEquals(1, fleet.getShips().size());
    }

    @Test
    @DisplayName("addShip: Falha ao exceder o tamanho da frota (FLEET_SIZE)")
    void testAddShip_Failure_FleetSizeLimit() {
        // Assume IFleet.FLEET_SIZE = 12 e BOARD_SIZE = 10

        // *** CORRIGIDO ***
        // Cria barcos numa grelha 5x5 espaçada por 2, o que garante
        // que não são adjacentes e cabem num tabuleiro 10x10.
        // (0,0), (0,2), (0,4), (0,6), (0,8)
        // (2,0), (2,2), (2,4), (2,6), (2,8)
        // (4,0), (4,2)
        for (int i = 0; i < IFleet.FLEET_SIZE; i++) {
            int row = (i / 5) * 2; // Linhas: 0, 0, 0, 0, 0, 2, 2, 2, 2, 2, 4, 4
            int col = (i % 5) * 2; // Cols: 0, 2, 4, 6, 8, 0, 2, 4, 6, 8, 0, 2
        ...
        assertEquals(IFleet.FLEET_SIZE, fleet.getShips().size(), "Frota deve estar cheia");

        // Tenta adicionar mais um (em posição válida)
        TestShip extraShip = new TestShip("Extra", Compass.NORTH, new Position(9, 9));
        extraShip.addTestPosition(new Position(9, 9));
        assertFalse(fleet.addShip(extraShip), "Não deve adicionar acima do FLEET_SIZE");
        assertEquals(IFleet.FLEET_SIZE, fleet.getShips().size(), "Tamanho da frota não deve aumentar");
    }
    ...
    void testGetFloatingShips() {
        ...
        // Afunda o ship1 (tem 2 posições: (1,1) e (1,2))
        ship1.getPositions().get(0).shoot();
        ship1.getPositions().get(1).shoot();
        assertFalse(ship1.stillFloating(), "Confirma que ship1 está afundado");

        List<IShip> floating = fleet.getFloatingShips();
        assertEquals(1, floating.size(), "Apenas ship2 deve flutuar");
        assertEquals(ship2, floating.get(0));

        // Afunda o ship2 (tem 1 posição: (5,5))
        ship2.getPositions().get(0).shoot();
        assertFalse(ship2.stillFloating(), "Confirma que ship2 está afundado");

        assertEquals(0, fleet.getFloatingShips().size(), "Nenhum barco deve flutuar");
    }
    ...
    void testShipAt_Found() {
        ...
        assertEquals(ship1, fleet.shipAt(new Position(1, 1)), "Deve encontrar ship1 em (1,1)");
        assertEquals(ship1, fleet.shipAt(new Position(1, 2)), "Deve encontrar ship1 em (1,2)");
        assertEquals(ship2, fleet.shipAt(new Position(5, 5)), "Deve encontrar ship2 em (5,5)");
    }
    ...
    void testShipAt_NotFound() {
        fleet.addShip(ship1);
        assertNull(fleet.shipAt(new Position(9, 9)), "Posição vazia deve retornar null");
        assertNull(fleet.shipAt(new Position(1, 3)), "Posição adjacente deve retornar null");
    }

    @Test
    @DisplayName("Cobertura de Asserção em printShipsByCategory (requer -ea)")
    void testAssertionCoverage() {
        // Redireciona System.out para não poluir a consola
    ...
    void testPrintMethods() {
        // Teste "smoke": apenas chama os métodos para garantir que não
        // dão crash e para obter cobertura de linha.
        // Redireciona a saída para não poluir a consola de teste.
        ...
        // Verifica se algo foi impresso (teste básico)
        String output = out.toString();

        // Verifica as strings que sabemos que DEVEM estar lá
        assertTrue(output.contains("[Barge n Linha = 1 Coluna = 1]"), "Output deve conter ship1");
        assertTrue(output.contains("[Caravel s Linha = 5 Coluna = 5]"), "Output deve conter ship2");
    ...
    void testGetShipsLike_EmptyFleet() {
        // 'fleet' é nova e vazia por defeito do setUp, mas vamos garantir
